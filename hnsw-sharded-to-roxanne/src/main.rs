...
fn main() {
  ...
  let base_path = base_graph.find_shortest_spanning_tree(medoid);

  for id in base_graph.ids() {
    cliques.insert(id, DashMap::new());
    clique_neighbors.insert(id, base_graph.get_out_neighbors(id).0.clone());
    ...
    batch.into_par_iter().for_each(|&path| {
      ...
      for (base_id_from, base_id) in base_path.iter().cloned() {
        ...
        clique_neighbors
          .get_mut(&base_id)
          .unwrap()
          .extend(other_graph.get_out_neighbors(other_node).0);
