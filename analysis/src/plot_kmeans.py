...
with open("out/graph_dists.json", "r") as f:
    graph_dists = {int(i): {int(j): v for j, v in d.items()} for i, d in json.load(f).items()}
...
pairwise_dists = np.linalg.norm(cluster_centers[:, np.newaxis] - cluster_centers, axis=2)
...
plt.savefig("out/kmeans_dists.histogram.webp", format='webp', bbox_inches='tight', dpi=150)
