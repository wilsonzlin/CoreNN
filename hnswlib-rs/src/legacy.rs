...
pub fn load_hnswlib<'a, M: Metric>(
  metric: M,
  dim: usize,
  bytes: &'a [u8],
...
mod tests {
  ...
  fn loads_real_hnswlib_fixture() {
    ...
    assert_eq!(v0.as_f32_slice(), &[0.0, 1.0, 2.0, 3.0]);
    let v7_id = graph.node_id(&7).unwrap();
    let v7 = vectors.vector(v7_id).unwrap();
    assert_eq!(v7.as_f32_slice(), &[70.0, 71.0, 72.0, 73.0]);
    ...
    for key in [0u64, 1, 2, 7, 31] {
      ...
      let hits = graph.search(&vectors, q.as_f32_slice(), 1, None).unwrap();
